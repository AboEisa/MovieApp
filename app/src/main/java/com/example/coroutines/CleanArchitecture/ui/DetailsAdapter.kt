...
class DetailsAdapter : RecyclerView.Adapter<DetailsAdapter.Holder>() {
    ...
    var movieDetails: PresentationMovieDetailsModel? = null
    ...
    inner class Holder(val binding: ItemDetailsBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(movieDetails: PresentationMovieDetailsModel) {
            binding.apply {
                ...
                Glide.with(binding.root)
                    .load(Constant.Companion.IMAGES +movieDetails.poster_path)
                    .into(moviePoster)
            }
        }
    }
